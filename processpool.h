...
//一个子进程的类，m_pid是目标子进程的pid,m_pipefd是父进程和子进程通信用的管道
class process{
public:
	process():m_pid(-1){}
public:
...
//从epollfd标示的epoll内核事件表中删除fd上的所有注册事件
...
void processpool<T>::run_child(){
	setup_sig_pipe();
	//每个子进程都通过其在进程池中的序号值m_idx找到与父进程的管道
	...
	while(!m_stop){
		...
		for(int i = 0; i < number; i++){
			...
			if((sockfd == pipefd) && (events[i].events & EPOLLIN)){
				int client = 0;
				//从父、子进程之间的管道读取数据，并将结果保存在变量client中，如果读取成功，则表示有新客户连接的到来
...
//模板类T必须实现init方法，以初始化一个客户连接，我们直接使用connfd编辑处理对象（T类型的对象），以提高程序效率
...
//如果是其他可读数据，必然是客户请求到来，调用逻辑处理对象的process方法处理之
...
//如果所有子进程都已经退出了，则父进程也退出
							...
							case SIGTERM:
							case SIGINT:{
/*如果父进程接收到终止信号，那么就杀死所有子进程，并等待它们全部结束。当然，通知子进程结束更好的方法是向父、子进程之间的通信管道发送特殊数据*/
