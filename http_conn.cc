...
const char* error_404_form = "The requested filed was not found on this server.\n";
...
http_conn::HTTP_CODE http_conn::do_request(){
	...
	if(!(m_file_stat.st_mode & S_IROTH)){//其他读
	...
	m_file_address = (char*)mmap(0,m_file_stat.st_size, PROT_READ,MAP_PRIVATE, fd, 0);//页内容可以被读取，建立一个写入时拷贝的私有映射
	close(fd);
	return FILE_REQUEST;
}
